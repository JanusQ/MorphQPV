...
@ray.remote
def build_accuracy(circuit,N_qubit,method='random',base_num=4):
    ...
    build_input, build_output = infer_process_by_statevector(layer_circuit[1:],input_state,method=method,base_num=base_num,target='input',optimizer='descent',learning_rate=0.01,steps=5000,jobname='build_accuracy',device='simulate')
    return fidelity(convert_state_to_density(input_state),convert_state_to_density(build_input))

def get_mean_fidelity(name,circuit,n_qubits,base_num=4,test_num=8,resultspath=None):
    acc_dict = [{} for _ in range(test_num)]
    for method in ['random','base']:
        fid = ray.get([build_accuracy.remote(circuit,n_qubits,method=method,base_num=base_num) for _ in tqdm(range(test_num),desc=f'{name}_{n_qubits}_{base_num} testing')])
        for i in range(test_num):
            acc_dict[i][method] = fid[i]

    with open(resultspath,'a') as f:
        for i in range(test_num):
            f.write(f'{name},{n_qubits},{base_num},{acc_dict[i]["base"]},{acc_dict[i]["random"]}\n')
    
def get_results(name,n_qubits,csvpath,test_num=8):
...
def plot_results(csvpath):
    ...
    for m,col in enumerate(data.columns[-2:]):
        ax.plot(data.samples,data[col]*100,marker='o',label=col,linewidth=3,markeredgewidth=5,markersize=25,color=colors[m])
    ax.legend(loc='upper left',fontsize=60)
    ax.set_xticks(data.samples)
    ax.set_xscale('log',base=2)
    ax.set_xticklabels(r'$2^{'+f'{int(np.log2(i))}'+'}$' for i in data.samples)
