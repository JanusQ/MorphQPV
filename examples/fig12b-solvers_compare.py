...
def plot_solver_time(csvpath):
    ...
    runtime_mean = runtime.groupby('qubits', as_index=False).mean().sort_values(by='qubits')
    runtime_std = runtime.groupby('qubits', as_index=False).std().sort_values(by='qubits')
    names = ['annealing',"descent","descent"]
    for m,col in tqdm(enumerate(runtime.columns[2:])):
        ax.plot(runtime_mean['qubits'], runtime_mean[col]*1e3, 'o-',markersize=markersize, color=colors[m],linewidth=linewidth)
        ax.errorbar(runtime_mean['qubits'], runtime_mean[col]*1e3, yerr=runtime_std[col]*1e3, fmt='o',markersize=markersize, color=colors[m],linewidth=linewidth,label=names[m])
...
if __name__ == '__main__':
    resultspath = f'examples/fig12(b)-solvers_compare/'
    if not os.path.exists(resultspath):
        os.mkdir(resultspath)
    csvpath = f'{resultspath}time.csv'
    get_solver_time(csvpath)
