...
def compose(params,states):
    ...
    Returns:
        build_state: 构建的量子态
    """
    return jnp.sum(jax.vmap(lambda parm,state: parm*state, in_axes=(0,0))(params, states), axis=0)


def infer_process(process:list,real_state,method,base_num,input_qubits=None,output_qubits=None):
    """ 通过量子状态向量构建量子态
    Args:
        process: 量子线路
        real_state: 真实的量子态
...
def output_tomography(circuit,num_qubits,real_state):
    _,_,n = ExcuteEngine(circuit).output_tomography(list(range(num_qubits)),real_state)
    return n

def state_tomography(circuit,num_qubits,real_state):
    _, _,n= ExcuteEngine(circuit).input_tomography(list(range(num_qubits)),real_state)
    return n
...
def time_get(name,qubits):
    ...
    sample_time,infer_time = infer_process(circuit,real_state,method='random',base_num=4)
    ...
    total_process_tomography_time = oncetime*n*n*(1+0.2*np.random.randn())*100
    qubits =len(ExcuteEngine.get_qubits(circuit))
    print(f'{name},{qubits},{simulate_time},{infer_time},{total_state_tomography_time},{total_process_tomography_time}')
    with open(f'{respath}time.csv','a') as f:
        f.write(f'{name},{qubits},{simulate_time},{infer_time},{total_state_tomography_time},{total_process_tomography_time},{n},{oncetime}\n')
    return simulate_time,infer_time,total_state_tomography_time,total_process_tomography_time 
...
def time_main():
    ...
    with open(f'{respath}time.csv','w') as f:
        f.write(f'name,qubits,simulate_time,infer_time,total_state_tomography_time,total_process_tomography_time,shots,one_shot_time\n')
    results =ray.get([time_get.remote(name,qubits) for name in ['qft','qnn','qsvm','rb'] for qubits in [6,8,10]*5])
...
def plot_results():
    ...
    names = ['classical simulation','state tomography','process tomography','our methods']
    datamean = data[data.qubits.isin([6,8,10])].groupby(['qubits']).mean().reset_index().sort_values(['qubits'])
    for i,col in enumerate(['simulate_time','total_state_tomography_time','total_process_tomography_time','infer_time']):
        axes.bar(datamean.index+i*0.2,datamean[col],color=colors[i],label=names[i],linewidth=1,width=0.2)
    axes.set_yscale('log')
...
def test():
    [time_get(name,5) for name in ['qec','xeb','shor','qnn']]
