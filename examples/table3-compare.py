...
from morphQPV.execute_engine.excute import ExcuteEngine,qiskit_circuit_to_layer_cirucit,convert_state_to_density
...
def gate_delete_generator(right_layer_circuit,delete_gate_num=1):
    ...
    for i in range(delete_gate_num):
        ...
        bad_layer_circuit[delete_gate_layer_idx].pop(np.random.randint(0,len(delete_gate_layer)))
...
def gate_add_generator(right_layer_circuit,n_qubits,_gate_num=1):
    ...
    for i in range(_gate_num):
        ...
        ## random choose a gate in this layer
        bad_layer_circuit[_layer_idx].insert(np.random.randint(0,len(_layer)),{'name':'z','qubits':[np.random.choice(n_qubits,size=1)[0]]})
...
def standard_test(assertionfunc,state_prepare,right_circuit,bad_circuit,num_qubits,input_num=10):
    for j in range(input_num):
        heads = state_prepare(num_qubits)
        real_output_state = ExcuteEngine.excute_on_pennylane(heads+right_circuit,type='statevector')
        verify_right,spend_gates = assertionfunc(heads,right_circuit,real_output_state,num_qubits)
        if not verify_right:
            return 0,spend_gates
        is_right,spend_gates = assertionfunc(heads,bad_circuit,real_output_state,num_qubits)
        if not is_right:
            return 1,spend_gates
    return 0,spend_gates


def standard_mean_test(assertionfunc,state_prepare,right_circuit,bad_circuit,num_qubits,total_round=10,input_num=3):
    res = np.mean(np.array([standard_test(assertionfunc,state_prepare,right_circuit,bad_circuit,num_qubits,input_num) for i in range(total_round)]),axis=0)
    return res

def proj_assertion(heads,layer_circuit,real_output_state,num_qubits):
    ## proj test
    proj_circuit,fake_gates = proj.assertion(real_output_state,list(range(num_qubits)))
...
def ndd_assertion(heads,layer_circuit,real_output_state,num_qubits):
    ## proj test
    proj_circuit,fake_gates,ancliqubit = ndd.assertion(real_output_state,list(range(num_qubits)))
    circuit_obj = ExcuteEngine(fake_gates)
    proj_gates = circuit_obj.gates_num
...
def test_assertion(name,num_qubits,assertionfunc=proj_assertion):
    ...
    if assertionfunc(heads,right_layer_circuit,ExcuteEngine.excute_on_pennylane(heads+right_layer_circuit,type='statevector'),num_qubits)[0]:
        print('test pass')
    else:
        print('test fail')
...
def profilling_methods(name,num_qubits):
    ...
    ## ndd test
    ndd_confidence,ndd_gates = standard_mean_test(ndd_assertion,standard_state_prepare,right_layer_circuit,bad_layer_circuit,num_qubits)
